      ...
      <div>
        ...
        <b-row >
          <b-col md=12>
            ...
            <b-row>
              <b-col md=6>
                Showing {{ paginations.eWalletAccount.from }} to {{ paginations.eWalletAccount.to }} of {{ paginations.eWalletAccount.totalRows }} records.
                </b-col>
              <b-col md=6>
                <b-pagination
...
export default {
    ...
    onEWalletAccountEntry(){
      const { eWalletAccount, eWalletAccount: { fields } } = this.forms
      const { eWalletAccounts } = this.tables
      ...
      if(this.entryMode == "Add"){
        this.addEWalletAccount(fields)
          .then(({ data }) => {
            ...
            showNotification(this, "success", "E-Wallet Account created successfully.")
        ...
        this.updateEWalletAccount(fields, fields.id)
          .then(({ data }) => {
            ...
            eWalletAccount.isProcessing = false
            showNotification(this, "success", "E-Wallet Account updated successfully.")
            this.showModalEntry = false
          })
          .catch(error => {
            const errors = error.response.data.errors
    ...
    onEWalletAccountDelete(){
      const { eWalletAccount, eWalletAccount: { fields: { id } } } = this.forms
      const { eWalletAccounts } = this.tables
      eWalletAccount.isProcessing = true
      this.deleteEWalletAccount(id)
        .then(({ data }) => {
          this.deleteRow(eWalletAccounts, this.paginations.eWalletAccount, id)
          eWalletAccount.isProcessing = false
          showNotification(this, "success", "E-Wallet Account deleted successfully.")
