      ...
      <b-card>
          ...
          <b-row>
            <b-col md="12">
                ...
                <b-row class="pb-1">
                  <b-col md="6">
                    ...
                    <span class="font-weight-bold">{{
                      forms.evaluation.fields.student.studentNo
                        ? forms.evaluation.fields.student.studentNo
                  ...
                  <b-col md="6">
                    Name :
                    <span class="font-weight-bold">{{
                      forms.evaluation.fields.student.name
                    }}</span>
                  </b-col>
                </b-row>
                <b-row class="pb-1">
                  <b-col md="6">
                    ...
                    <b-badge
                      :variant="
                        forms.evaluation.fields.studentCategoryId ===
                    ...
                    >
                      {{
                        forms.evaluation.fields.studentCategory
                          ? forms.evaluation.fields.studentCategory.name
                          : ''
                      }}
                    </b-badge>
                  </b-col>
                  <b-col md="6">
                    Contact Number :
                    <span class="font-weight-bold">{{
                      forms.evaluation.fields.student.mobileNo
                    }}</span>
                  </b-col>
                </b-row>
                <b-row class="pb-1">
                  <b-col md="6">
                    Email :
                    <span class="font-weight-bold">{{
                      forms.evaluation.fields.student.email
                    }}</span>
                  </b-col>
                  <b-col md="6">
                    Address :
                    <span class="font-weight-bold">
                      {{
                        forms.evaluation.fields.student.address
                          ? forms.evaluation.fields.student.address
              ...
              <b-row class="my-2 pb-1">
                <b-col md="12">
                  <span class="font-weight-bold"
                    >Previous Educational Background</span
                  >
                  <hr class="bg-light mt-1 mb-0" />
                </b-col>
              </b-row>
                    ...
                    >
                      <template v-slot:first>
                        <b-form-select-option :value="null" disabled
                          >-- School Level --</b-form-select-option
                        >
                      </template>
              ...
              <b-row class="my-2 pb-1 ">
                <b-col md="12">
                  <span class="font-weight-bold"
                    >Applying for Academic Record</span
                  >
                  <hr class="bg-light mt-1 mb-0" />
                </b-col>
              </b-row>
              <b-row class="pb-1">
                ...
                <b-col md="3">
                  <b-form-group>
                    <label>Level</label>
                    <b-form-select v-model="forms.evaluation.fields.levelId">
                      <template v-slot:first>
                        <b-form-select-option :value="null" disabled
              ...
              <b-row v-if="forms.evaluation.fields.course">
                <b-col md="6">
                  <b-form-group>
                    ...
                    <b-form-select
                      @change="
                        $event === null
                          ? (forms.evaluation.fields.semesterId = null)
                          : ''
                      "
                ...
                <b-col md="3">
                  <b-form-group>
                    ...
                    <b-form-select v-model="forms.evaluation.fields.semesterId">
                      <template v-slot:first>
                        <b-form-select-option :value="null" disabled
        ...
        <b-card>
          <b-overlay :show="isLoadingCurriculum">
            ...
            <div v-if="forms.transcriptRecord.fields.subjects">
                ...
                <b-col md="12">
                  <b-row>
                    <b-col md="6">
                      <h5>
                        {{
                          forms.evaluation.fields.level
                            ? forms.evaluation.fields.level.name
                            : ''
                        }}
                      </h5>
                    </b-col>
    ...
    >
      ...
      <b-row>
        <b-col md="12">
          ...
          <b-row>
            <b-col md="6">
              Showing {{ paginations.addSubject.from }} to
              {{ paginations.addSubject.to }} of
              {{ paginations.addSubject.totalRows }} records.
            </b-col>
            <b-col md="6">
              <b-pagination
  ...
  methods: {
    onUpdateEvaluation() {
      const {
        evaluation: {
          fields,
          fields: { student, level, course, studentCategory, ...evaluation },
        },
        transcriptRecord: {
          fields: {
            subjects: transcriptRecordSubjects,
            ...transcriptRecordData
          },
        },
      } = this.forms;

      copyValue(evaluation, transcriptRecordData);

      const subjects = transcriptRecordSubjects.map((s) => {
        const subject = {
          subjectId: s.id,
      ...
      this.updateEvaluation(data, evaluation.id)
        .then(({ data }) => {
          ...
          this.$router.push({
            path: `/master-files/student/${this.$route.params.studentId}/school-records/evaluations`,
            query: { success: true },
          });
        })
        .catch((error) => {
    ...
    loadEvaluation() {
      ...
      const {
        evaluation: { fields },
        transcriptRecord: { fields: transcriptRecordFields },
      } = this.forms;
      clearFields(fields);
      clearFields(transcriptRecordFields);
      transcriptRecordFields.subjects = [];
      this.getEvaluation(this.evaluationId).then(({ data }) => {
        const transcriptRecordId = data.transcriptRecordId;
        copyValue(data, fields);
        console.log(fields);
        this.getSubjectsOfTranscriptRecord(transcriptRecordId, {
          paginate: false,
        }).then(({ data }) => {
          transcriptRecordFields.subjects = data;
    ...
    loadLevelsOfCourse() {
      ...
      this.getLevelsOfCourse(
        this.forms.evaluation.fields.courseId,
        params
      ).then(({ data }) => {
        levels.items = data;
        this.isLoadingCurriculum = false;
