    ...
    <b-overlay :show="isLoading" rounded="sm">
      <b-card>
        <div v-if="forms.academicRecord.fields.student">
          ...
          <b-row class="pb-1">
            <b-col md=6>
              Student Number : <span class="font-weight-bold">{{ forms.academicRecord.fields.student.studentNo ? forms.academicRecord.fields.student.studentNo : 'Awaiting Confirmation' }}</span>
            </b-col>
            <b-col md=6>
              Name : <span class="font-weight-bold">{{ forms.academicRecord.fields.student.name }}</span>
            </b-col>
          </b-row>
          <b-row class="pb-1">
            <b-col md=6>
              Student Category :
              <b-badge
                :variant="forms.academicRecord.fields.studentCategoryId === $options.StudentCategories.NEW.id
                  ? 'success'
                  : forms.academicRecord.fields.studentCategoryId === $options.StudentCategories.OLD.id ? 'primary' : 'warning'">
                {{ forms.academicRecord.fields.studentCategory ? forms.academicRecord.fields.studentCategory.name : "" }}
              </b-badge>
            </b-col>
            <b-col md=6>
              Contact Number : <span class="font-weight-bold">{{ forms.academicRecord.fields.student.mobileNo }}</span>
            </b-col>
          </b-row>
          <b-row class="pb-1">
            <b-col md=6>
              Email : <span class="font-weight-bold">{{ forms.academicRecord.fields.student.email }}</span>
            </b-col>
            <b-col md=6>
              Address :
              <span class="font-weight-bold">
                {{ forms.academicRecord.fields.student.address ? forms.academicRecord.fields.student.address.currentCompleteAddress : "" }}
              </span>
            </b-col>
          </b-row>
        </div>
          ...
          <b-table
            ...
            <template v-slot:custom-foot>
              <b-tr class="font-weight-bold">
                ...
                <b-td class="text-center">
                  <span class="text-danger">
                      {{ totalUnits(forms.academicRecord.fields.subjects, 'totalUnits') }}
                  </span>
                </b-td>
                <b-td></b-td>
              </b-tr>
            ...
            <template v-slot:cell(action)="row">
              <b-button
                @click="removeSubject(forms.academicRecord.fields.subjects, row)"
        ...
        <div v-if="forms.academicRecord.fields.studentFee">
          ...
          <b-table
            ...
            <template v-slot:cell(pivot.amount)="row">
              <vue-autonumeric
                :disabled="row.item.id === $options.Fees.TUITION_FEE.id && forms.academicRecord.fields.isComputedByUnits === 1"
          ...
          <b-row>
            <b-col md=4>
              <b-form-group
                ...
                <vue-autonumeric
                  class="form-control text-right"
                  v-model="forms.academicRecord.fields.studentFee.enrollmentFee"
                  :options="[{ minimumValue: 0, modifyValueOnWheel: false, emptyInputBehavior: 0 }]">
                </vue-autonumeric>
              </b-form-group>
            </b-col>
            ...
            <b-col md=2>
              <h5 class="float-right font-weight-bold">{{ feesTotalAmount(forms.academicRecord.fields.studentFee, forms.academicRecord.fields.studentFee.studentFeeItems) }}</h5>
            </b-col>
          </b-row>
        </div>
        <template v-slot:footer>
          <b-button
            :to="`/master-files/student/${$route.params.studentId}/school-records/enrollments`"
...
<script>
import { AcademicRecordStatuses, StudentCategories, Semesters, Fees } from '../../../helpers/enum'
import { copyValue } from '../../../helpers/extractor'
import { formatNumber, showNotification } from '../../../helpers/forms'
import tables from '../../../helpers/tables'
import { AcademicRecordApi, CourseApi, LevelApi, SchoolFeeApi, SectionApi, SubjectApi } from '../../../mixins/api'
...
export default {
  ...
  Semesters,
  Fees,
  mixins: [ AcademicRecordApi, SectionApi, CourseApi, LevelApi, SubjectApi, SchoolFeeApi, tables ],
  created() {
  ...
  methods: {
    onUpdateEnrollment() {
      ...
      .then(({ data }) => {
        ...
        this.$router.push({path: `/master-files/student/${this.$route.params.studentId}/school-records/enrollments`, query: { success: true } })
      }).catch((error) => {
    ...
    loadAcademicRecord() {
      ...
      this.getAcademicRecord(this.academicRecordId)
        .then(({ data }) => {
    ...
    onAddSubject(item){
      ...
      const result = studentFee.studentFeeItems.find(fee => fee.id === item.id)
