      ...
      <div>
        ...
        <b-row >
          <b-col md=12>
            <b-table
              ...
              <template v-slot:cell(action)="row">
                <b-dropdown right variant="link" toggle-class="text-decoration-none" no-caret>
                  ...
                  <b-dropdown-item
                    @click="forms.bankAccount.fields.id = row.item.id, showModalConfirmation = true"
            ...
            <b-row>
              <b-col md=6>
                Showing {{ paginations.bankAccount.from }} to {{ paginations.bankAccount.to }} of {{ paginations.bankAccount.totalRows }} records.
                </b-col>
              <b-col md=6>
                <b-pagination
...
export default {
  ...
  components: {
    ...
    onBankAccountEntry(){
      const { bankAccount, bankAccount: { fields } } = this.forms
      const { bankAccounts } = this.tables
      ...
      if(this.entryMode == "Add"){
        this.addBankAccount(fields)
          .then(({ data }) => {
            ...
            showNotification(this, "success", "Bank Account created successfully.")
        ...
        this.updateBankAccount(fields, fields.id)
          .then(({ data }) => {
            ...
            bankAccount.isProcessing = false
            showNotification(this, "success", "Bank Account updated successfully.")
            this.showModalEntry = false
          })
          .catch(error => {
            const errors = error.response.data.errors
    ...
    onBankAccountDelete(){
      const { bankAccount, bankAccount: { fields: { id } } } = this.forms
      const { bankAccounts } = this.tables
      bankAccount.isProcessing = true
