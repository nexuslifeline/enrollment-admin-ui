      ...
      <div>
        ...
        <b-row >
          <b-col md=12>
            <b-table
              ...
              <template v-slot:cell(action)="row">
                <b-dropdown right variant="link" toggle-class="text-decoration-none" no-caret>
                  ...
                  <b-dropdown-item
                    @click="forms.peraPadalaAccount.fields.id = row.item.id, showModalConfirmation = true"
            ...
            <b-row>
              <b-col md=6>
                Showing {{ paginations.peraPadalaAccount.from }} to {{ paginations.peraPadalaAccount.to }} of {{ paginations.peraPadalaAccount.totalRows }} records.
                </b-col>
              <b-col md=6>
                <b-pagination
...
export default {
    ...
    onPeraPadalaAccountEntry(){
      const { peraPadalaAccount, peraPadalaAccount: { fields } } = this.forms
      const { peraPadalaAccounts } = this.tables
      ...
      if(this.entryMode == "Add"){
        this.addPeraPadalaAccount(fields)
          .then(({ data }) => {
            this.addRow(peraPadalaAccounts, this.paginations.peraPadalaAccount, data)
            peraPadalaAccount.isProcessing = false
            showNotification(this, "success", "Pera Padala created successfully.")
        ...
        this.updatePeraPadalaAccount(fields, fields.id)
          .then(({ data }) => {
            ...
            peraPadalaAccount.isProcessing = false
            showNotification(this, "success", "Pera Padala updated successfully.")
            this.showModalEntry = false
          })
          .catch(error => {
            const errors = error.response.data.errors
    ...
    onPeraPadalaAccountDelete(){
      const { peraPadalaAccount, peraPadalaAccount: { fields: { id } } } = this.forms
      const { peraPadalaAccounts } = this.tables
      peraPadalaAccount.isProcessing = true
      this.deletePeraPadalaAccount(id)
        .then(({ data }) => {
          this.deleteRow(peraPadalaAccounts, this.paginations.peraPadalaAccount, id)
          peraPadalaAccount.isProcessing = false
